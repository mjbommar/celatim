...
| Survivability E5/E6 (`run_mbox.py`, `run_taps.py`) | real kernel + real bump-in-the-wire forwarder | **L2** | **yes** — `pass` mode (E6) is the no-scrub control; 3-tap localizes the death | — |
| NAT E7 (`run_nat.py`) | real Linux MASQUERADE router | **L2** | needs a non-NAT control run to be complete | finding: IP-ID survives Linux NAT |
...
| Subliminal G — **2 mechanisms**: `ecdsa-nonce` (`run_subliminal.py`, `ecdsa` lib) and `rsa-pss-salt` (`run_subliminal_rsa.py`, pure-stdlib RSA + RFC 8017 EMSA-PSS) | standalone real crypto — not the kernel | **L2** (we choose k / the salt); real ECDSA sign+recover, and a real RSA signature `s=EM^d mod n` with PSS salt recovered from `s^e mod n` | **yes** — random-k / random-salt (honest signer) ⇒ no recoverable payload | round-trips or fails, no model |
